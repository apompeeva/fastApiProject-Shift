...
async def create_new_salary_data(salary_data: SalaryDataCreate, session: AsyncSession = Depends(get_async_session)):
    new_salary_info = await create_salary_data(salary_data, session)
    return new_salary_info


@salary_router.get("/salary_data")
async def get_salary_data(session: AsyncSession = Depends(get_async_session), user: User = Depends(current_user)):
